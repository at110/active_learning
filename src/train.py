...
def entropy_volume(vol_input:torch.Tensor) -> torch.Tensor:
        """
        Calculate the entropy of a given volumetric input tensor.

        The function computes the entropy over the channel dimension of the input tensor,
        assuming the input represents a probability distribution over classes (channels)
        at each voxel in a volumetric grid. The entropy is calculated for each voxel
        individually, resulting in a tensor of entropy values with the same spatial
        dimensions as the input.

        Parameters:
        - vol_input: A torch.Tensor representing probability distributions over classes
                    for each voxel in a volumetric grid. The tensor should have the
                    shape [MC samples, Channels, *Spatial dimensions], where 'MC samples'
                    is the number of Monte Carlo simulations, 'Channels' is the number of
                    classes, and '*Spatial dimensions' represents the dimensions of the
                    volumetric grid (e.g., Depth x Height x Width for 3D volumes).

        Returns:
        - A torch.Tensor containing the computed entropy values for each voxel, with the
            shape [*Spatial dimensions], matching the spatial dimensions of the input tensor.
        """
        ...
        for channel in range(dims[1]):
            t_vol = torch.squeeze(vol_input[:, channel, :, :, :])
...
def select_data_by_uncertainty_with_sw_inference(
    ...
    for data in data_loader:
        ...
        mc_entropy = entropy_volume(mc_predictions)
...
def run_training(model: torch.nn.Module, train_loader: torch.utils.data.DataLoader, 
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % val_interval == 0:
            val_loss, metric = validate(model, val_loader, loss_function, device)
...
def main():
    ...
    test_loss, test_metric = validate(model, loaders["test"], loss_function, device)
    mlflow.log_metric("test_loss", test_loss)
    mlflow.log_metric("test_dice", test_metric)
